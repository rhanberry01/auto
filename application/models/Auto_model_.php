...
class Auto_model_ extends CI_Model {
     ...
     public function sample(){
        ...
        $this->ddb = $this->load->database('133', true);
                     ...
                     $sql1 = " insert into possible_no_display (date_,productid,descripiton,currentinventory,past30days,average,past2days)                   
                                ...
                                where cast(logdate as date) >= '".$end_date."' and cast(logdate as date) <= '".$end_date."' and fs.voided = 0
        ...
        return $end_date.'--'.$past_30days.'--'.$past_2days.'--'.$end_past_2days.'-->';
     }

     public function get_po(){
        $end_date =  date('Y-m-d'); 
        $past_5days = date('Y-m-d',strtotime('today - 15 days')); 
        ...
        $sql1 = "SELECT a.PurchaseOrderNo,a.ReceivingNo, 'srsn' as Branch,
                ...
                WHERE cast(a.DateReceived as date) >= '".$past_5days."' and cast(a.DateReceived as date) <= '".$end_date."' and
	...
	$sql1 = "SELECT a.PurchaseOrderNo,a.ReceivingNo, 'sri' as Branch,
                a.DateReceived,SUM(b.extended)AS tot_extended,b.ReceivingID
                FROM ROF a 
                JOIN ROFLine b ON (a.ReceivingID = b.ReceivingID)
                WHERE cast(a.DateReceived as date) >= '".$past_5days."' and cast(a.DateReceived as date) <= '".$end_date."' and
                a.PurchaseOrderNo !='' and a.PurchaseOrderNo != '0' and a.PurchaseOrderNo IS NOT NULL
                GROUP BY b.ReceivingID,a.ReceivingNo,a.DateReceived,a.PurchaseOrderNo";
      ...
      public function insert_free_po($PurchaseOrderNo,$ReceivingNo,$Branch,$DateReceived,$tot_extended,$ReceivingID){

        $this->ddb = $this->load->database('po_received_server', true);

        $sql1 = "INSERT IGNORE INTO free_purchases(PurchaseOrderNo,ReceivingNo,Branch,DateReceived,tot_extended,ReceivingID)
                VALUES('".$PurchaseOrderNo."','".$ReceivingNo."','".$Branch."','".$DateReceived."',$tot_extended,'".$ReceivingID."')";
	...
	$sql1 = "SELECT a.ReferenceNo, a.MovementNo, 'sri' as Branch, 
			a.PostedDate,SUM(b.extended)AS tot_extended, b.MovementID from Movements a
 		JOIN MovementLine b ON (a.MovementID = b.MovementID) 
		WHERE a.ReferenceNo !='' and a.ReferenceNo != '0' 
			and a.ReferenceNo IS NOT NULL
 		AND MovementCode = 'PSV' AND cast(a.PostedDate as date) >= '".$past_5days."' and cast(a.PostedDate as date) <= '".$end_date."' 
		GROUP BY b.MovementID,a.MovementNo,a.PostedDate,a.ReferenceNo";
      ...
      public function insert_survey_po($PurchaseOrderNo,$ReceivingNo,$Branch,$DateReceived,$tot_extended,$ReceivingID){

        $this->ddb = $this->load->database('po_received_server', true);

        $sql1 = "INSERT IGNORE INTO price_survey_purchases(PurchaseOrderNo,ReceivingNo,Branch,DateReceived,tot_extended,ReceivingID)
                VALUES('".$PurchaseOrderNo."','".$ReceivingNo."','".$Branch."','".$DateReceived."',$tot_extended,'".$ReceivingID."')";
    ...
    // ON DUPLICATE KEY UPDATE   years = ".$details['years']." , months = ".$details['months'].", vat_cost = ".$details['vat_cost']." ,vat_sales =  ".$details['vat_sales']." ,non_vat_cost = ".$details['non_vat_cost']." ,non_vat_sales = ".$details['non_vat_sales'].",zero_vat_cost = ".$details['zero_vat_cost']." ,zero_vat_sales = ".$details['zero_vat_sales']." ,total_sales = ".$details['total_sales'].",total_cost = ".$details['total_cost']." ,sukipoints = ".$details['sukipoints']." 
...
$sql="INSERT INTO consolidated_inventory 
	...
	public function insert_movements($db,$datefrom,$dateto){
         $this->ddb = $this->load->database($db, true);

         $sql="INSERT INTO
			consolidated_movements
			SELECT  YEAR(cast(PostedDate  as date)) as years,MONTH(cast(PostedDate as  date)) as months,movements.movementcode, MovementTypes.Description,
			ROUND(SUM(CASE
			...
			where CAST (Movements.PostedDate  as DATE) between '".$datefrom."' and  '".$dateto."' and  Movements.status = 2
			group by  MONTH(cast(PostedDate as  date)),YEAR(cast(PostedDate as date)),movements.movementcode, MovementTypes.Description";
